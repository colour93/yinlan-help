...
!!!tip "本协议适用于玖叁的群聊机器人服务 （FurBot - 洇岚，以下简称“洇岚”或“本服务”），协议可能会由于功能或政策变化进行更新。 最近更新日期：2022年4月22日"

    如您在阅读本协议后存在异议，或不同意此协议条款中的有关内容，请与开发者联系，并暂时停止使用 洇岚 提供的任何服务，将其**移出您的群聊**。
...
1. 洇岚 是一个 Furry 兽圈文化群聊机器人，服务对象主体为 **Furry 文化相关群聊及群员**，如发现服务对象不匹配的情况，{==经查证后 洇岚 将主动停止所有服务，并禁止该群聊继续使用洇岚有关内容的权限，**洇岚将主动停止所有服务，退出相关群聊并将其加入黑名单**==}；

2. 用户{==（指 洇岚 服务的群聊内部的所有成员）==}不得滥用 洇岚 所提供的服务（如进行诽谤、侮辱他人，刷屏等操作），如发现上述情况，{==经发现后将通过机器人进行警告，如在警告发生后仍存在对应情况，**洇岚将主动停止所有服务，退出相关群聊**==}；

3. {++开发者将不定期登录 Bot 进行**人工巡检**，如果您的群聊为私密或不能接受该项，**洇岚将主动停止所有服务，退出相关群聊**；++}

4. {++在 洇岚 进行功能性更新时（±1小时），**开发者将登录 Bot 进行功能引导、调试**，届时**开发者会以自然人身份使用 洇岚 的账号**，包括但不限于对用户的使用引导、在群内进行聊天等操作，如您不能接受，**洇岚将主动停止所有服务，退出相关群聊**；++}

5. {++如果您对 洇岚 或其开发者有看法或是意见，请您立即联系开发者，而不是对着一堆代码撒气，谢谢，顺带**立即将 洇岚 移出群聊**；++}

6. 用户发言应当遵守我国的社会主义核心价值观，遵守中华人民共和国及当地的法律法规，如有违反上述的情况，{==经发现后将通过机器人进行警告，如在警告发生后仍存在对应情况，**洇岚将主动停止所有服务，退出相关群聊**==}。

## ==2. 用户责任与义务==

1. 用户{==有责任==}保证，其向洇岚上传或发送的内容不得并禁止直接或间接的：
...
3. {++如用户为相关群聊的群主或管理员，还应承担以下责任：++}
    ...
    3. {++主动在洇岚**提供服务前**告知群聊成员有关本服务的用户协议及隐私政策，并**确认群聊成员已知情并同意**的责任；++}

    4. {++在群聊成员对洇岚有关服务存在异议并主动联系群主或管理员的情况下，主动停止使用本服务，并**将洇岚移出群聊的责任**。++}
...
2. {++开发者承诺此信息不会交由任何第三方，仅用于提供洇岚正常服务所需，所有信息在服务器端及本地的存储时限不会超过**7天**；++}
...
!!!info "请继续阅读洇岚的[《隐私政策》](/privacy)"

## 一些想法

这玩意写着真累，总的来说，开发者绝对不会做昧良心的事情，开发者所提供的服务也建立于大家对开发者的信任基础之上。